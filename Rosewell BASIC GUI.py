...
''')
    ...
    #Color Help
    elif cmd in ['Color Help','color help','COLOR HELP','Color help','color Help','ch','CH','Ch','cH']:
...
''')
    ...
    #Color Changing Feature
        ...
        if colors in ['BASIC','Basic','basic','Default','default','DEFAULT','B','b','d','D']:
            shell = Shell(root, highlightbackground ="#0F5FB4",highlightcolor="#0F5FB4", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#78DAFC", bg="#0F5FB4")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        
        if colors in ['c64','C64','Commodore 64','commodore 64','COMMODORE 64','C','c','64']:        
            shell = Shell(root, highlightbackground ="#6f6ed1",highlightcolor="#6f6ed1", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#6f6ed1", bg="#3F359B")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['c64 white','C64 white','Commodore 64 white','commodore 64 white','COMMODORE 64 white','Cw','cw','64w']:
            shell = Shell(root, highlightbackground ="#6f6ed1",highlightcolor="#6f6ed1", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#3F359B")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Red','RED','red','Dark Red','DARK RED','dark red','Dark red','dark Red','R','r','DR','dr','Dr','dR']:
            shell = Shell(root, highlightbackground ="#8B0D0D",highlightcolor="#8B0D0D", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#E39693", bg="#8B0D0D")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Red white','RED white','red white','Dark Red white','DARK RED white','dark red white','Dark red white','dark Red white','Rw','rw','DRw','drw','Drw','dRw']:
            shell = Shell(root, highlightbackground ="#8B0D0D",highlightcolor="#8B0D0D", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#8B0D0D")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Blue white','blue white','BLUE white','Dark Blue white','Dark blue white','dark Blue white','DARK BLUE white','dark blue white','DBw','dbw','Dbw','dBw']:
            shell = Shell(root, highlightbackground ="#10145A",highlightcolor="#10145A", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#10145A")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Blue','blue','BLUE','Dark Blue','Dark blue','dark Blue','DARK BLUE','dark blue','DB','db','Db','dB']:
            shell = Shell(root, highlightbackground ="#10145A",highlightcolor="#10145A", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#8A8FEA", bg="#10145A")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()   
        if colors in ['Teal','teal','TEAL','T','t']:
            shell = Shell(root, highlightbackground ="#008080",highlightcolor="#008080", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#52FFC5", bg="#008080")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Teal white','teal white','TEAL white','Tw','tw']:
            shell = Shell(root, highlightbackground ="#008080",highlightcolor="#008080", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#008080")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Orange','orange','ORANGE','O','o']:
            shell = Shell(root, highlightbackground ="#ff8c00",highlightcolor="#ff8c00", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#955E00", bg="#ff8c00")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Orange white','orange white','ORANGE white','Ow','ow']:
            shell = Shell(root, highlightbackground ="#ff8c00",highlightcolor="#ff8c00", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#ff8c00")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['green','GREEN','Green','G','g',]:
            shell = Shell(root, highlightbackground ="#247618",highlightcolor="#247618", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#A4EB99", bg="#247618")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['green white','GREEN white','Green white','Gw','gw',]:
            shell = Shell(root, highlightbackground ="#247618",highlightcolor="#247618", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#247618")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['gray','GRAY','Gray','GR','gr','Gr','gR']:
            shell = Shell(root, highlightbackground ="#525252",highlightcolor="#525252", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#525252")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['purple','PURPLE','Purple','P','p']:
            shell = Shell(root, highlightbackground ="#6A19A4",highlightcolor="#6A19A4", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#B770E9", bg="#6A19A4")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['purple white','PURPLE white','Purple white','Pw','pw']:
            shell = Shell(root, highlightbackground ="#6A19A4",highlightcolor="#6A19A4", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#6A19A4")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['yellow','YELLOW','Yellow','y','Y']:
            shell = Shell(root, highlightbackground ="#BCFE54",highlightcolor="#BCFE54", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#687312", bg="#BCFE54")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['yellow white','YELLOW white','Yellow white','yw','Yw']:
            shell = Shell(root, highlightbackground ="#BCFE54",highlightcolor="#BCFE54", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#000000", bg="#BCFE54")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['PINK','pink','Pink','pi','PI','Pi','pI']:
            shell = Shell(root, highlightbackground ="#ED33C4",highlightcolor="#ED33C4", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#900D74", bg="#ED33C4")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['PINK white','pink white','Pink white','pi white','PI white','Pi white','pI white']:
            shell = Shell(root, highlightbackground ="#ED33C4",highlightcolor="#ED33C4", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#000000", bg="#ED33C4")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['white','WHITE','White','w','W']:
            shell = Shell(root, highlightbackground ="#F6F6F6",highlightcolor="#F6F6F6", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#000000", bg="#F6F6F6")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['BLACK','black','Black']:
            shell = Shell(root, highlightbackground ="#000000",highlightcolor="#000000", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#F6F6F6", bg="#000000")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['vic20','Vic20','VIC20','vic-20','VIC-20','Vic-20','vic','VIC','Vic']:        
            shell = Shell(root, highlightbackground ="#00F9F7",highlightcolor="#00F9F7", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#061FEE", bg="#F6F6F6")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['plus','PLUS','Plus','Plus4','PLUS4','plus4','plus-4','PLUS-4','Plus-4']:        
            shell = Shell(root, highlightbackground ="#B69DF8",highlightcolor="#B69DF8", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#000000", bg="#F6F6F6")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['128','c128','C128','COMMODORE128','commodore128','Commodore128','Commodore 128',' commodore 128','COMMODORE 128']:        
            shell = Shell(root, highlightbackground ="#A1E08E",highlightcolor="#A1E08E", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#A1E08E", bg="#555555")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['PET','pet','Pet','pEt','peT','PEt','pET','PeT','commodore pet','Commodore pet','COMMODORE pet','COMMODORE PET','Commodore PET','commodore PET']:        
            shell = Shell(root, highlightbackground ="#000000",highlightcolor="#000000", highlightthickness=30, borderwidth=14, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#00FE52", bg="#000000")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
        if colors in ['Amber','AMBER','amber','A','a','Ab','aB','AB','ab']:
            shell = Shell(root, highlightbackground ="#000000",highlightcolor="#000000", borderwidth=10, width=69, height=30, font=('Atari-Basic.ttf', 14), fg="#ff8c00", bg="#000000")
            self.destroy()
            shell.pack(fill=tk.BOTH, expand=1)
            root.update()
...
''')
    ...
    #Text To Speech
    elif cmd in ['Speech','speech','SPEECH','Text To Speech','text to speech','TEXT TO SPEECH','Text to speech','text To speech','text to Speech',' Text To speech','text To Speech','Text to Speech','TTS','tts','Tts','tTs','ttS','TTs','tTS','TtS']:
